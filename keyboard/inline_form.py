...
def send_paginated_buttons(page, number_cell=None, button_list_domestic=None, is_city=None):
    ...
    if end_idx >= len(button_list):
        free_button = types.InlineKeyboardButton(text="Custom", callback_data=f'add_option:null:{number_cell}')
        keyboard.insert(free_button)
    if start_idx in list(range(len(button_list)))[1:-3]:
